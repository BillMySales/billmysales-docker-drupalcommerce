#!/bin/bash
# Backups of the Drupal database and its files (public and private). The
# hash salt (DRUPAL_HASH_SALT, in .env) isn't in them: keep it.
#
#   backup.sh            # loop: back up now, then every BACKUP_INTERVAL_HOURS
#   backup.sh now        # one backup
#   backup.sh list       # list backups
#   backup.sh restore <timestamp>   # restore database and files
...
# (files/ and private/), deleted after BACKUP_KEEP_DAYS days.
set -euo pipefail
...
backup() {
    ...
    tar -C "${DATA_DIR}" -czf "${tmp}-files.tar.gz" files private
...
restore() {
    ...
    find "${DATA_DIR}/files" "${DATA_DIR}/private" -mindepth 1 -maxdepth 1 -exec rm -rf {} +
